...
function GravityStarsBackground({
  ...
  const prevSizeRef = React.useRef({ width: 0, height: 0 }); // Track previous size
  ...
  // Removed redistributeStars as we now scale positions
  ...
  const resizeCanvas = React.useCallback(() => {
    ...
    // Scale stars if we have a previous size and stars exist
    if (starsRef.current.length > 0 && prevSizeRef.current.width > 0 && prevSizeRef.current.height > 0) {
      const scaleX = rect.width / prevSizeRef.current.width;
      const scaleY = rect.height / prevSizeRef.current.height;
      starsRef.current.forEach((p) => {
        p.x *= scaleX;
        p.y *= scaleY;
      });
    } else if (starsRef.current.length === 0) {
      // Only init if no stars exist
      initStars(rect.width, rect.height);
    }

    // Update previous size and state
  ...
  React.useEffect(() => {
    ...
    const touch = (e) => handlePointerMove(e);
